...
export async function POST(request: Request) {
  if (!isExperimentalRunsModeEnabled()) {
  ...
  const result = await runHermesRunsExperimentalChat(
    {
    ...
    parsed.request,
    {
      expectedText: parsed.expectedText ?? undefined,
      experimentalEnabled: true,
      memoryScopeBridgeEnabled: isMemoryScopeBridgeEnabled(),
      timeoutMs: 30_000
    }
  );
...
type ParseResult =
  | { ok: true; request: HermesChatRequest; expectedText: string | null }
...
function disabledResponse() {
  return {
    ...
    route: "/api/hermes/runs/experimental-chat",
    error: {
      kind: "disabled",
      message: "Experimental Hermes Runs mode is disabled. Set HERMES_UI_EXPERIMENTAL_RUNS_MODE=true for local/dev testing."
    },
    safety: {
      route: "bff-only",
...
async function readChatRequest(request: Request): Promise<ParseResult> {
  ...
  if (raw.length > MAX_BODY_BYTES) {
    return badRequest("bad_response", "Experimental Runs chat request is too large.");
  }
  ...
  try {
    ...
    return badRequest("bad_response", "Experimental Runs chat request body must be valid JSON.");
  }

  if (!body || typeof body !== "object" || Array.isArray(body)) {
    return badRequest("bad_response", "Experimental Runs chat request body must be an object.");
  }
  ...
  return {
    ok: true,
    expectedText: cleanOptionalString(input.expectedText, 256),
    request: {
      context,
...
function isExperimentalRunsModeEnabled(): boolean {
  return process.env.HERMES_UI_EXPERIMENTAL_RUNS_MODE === "true";
}
