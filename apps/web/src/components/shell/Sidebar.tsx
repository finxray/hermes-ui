...
function CollapsibleSessionList({
  ...
  const visibleSessions = sessions.slice(0, VISIBLE_CHAT_LIMIT);
  const overflowSessions = sessions.slice(VISIBLE_CHAT_LIMIT);
  const hasOverflow = overflowSessions.length > 0;

  useEffect(() => {
    if (!activeSessionId || !hasOverflow) {
      return;
    }
    const activeIndex = sessions.findIndex((session) => session.id === activeSessionId);
    if (activeIndex >= VISIBLE_CHAT_LIMIT) {
      setExpanded(true);
    }
  }, [activeSessionId, hasOverflow, sessions]);
