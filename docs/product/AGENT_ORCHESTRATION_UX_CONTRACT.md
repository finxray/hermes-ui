...
| Command execution block | Show shell or command tool execution. | command, cwd, status, stdout/stderr preview, exit code, duration. | Tool events from Hermes/MCP tools when payload identifies command execution. | Compact row, expandable stdout/stderr. | Partially implemented | 13L |
...
| Stdout/stderr preview | Show command output safely. | stdout, stderr, truncation, exit code. | Command/tool event payloads. | Redacted previews inline; raw details collapsed. | Partially implemented | 13L |
