...
assert(sidebar.includes('sidebarOrganization === "project" ? channelGroups.map'));
assert(sidebar.includes("visibleOneListEntries.map"));
assert(sidebar.includes('entry.kind === "channel"'));
assert(!sidebar.includes("<ul className={styles.list} hidden={projectsCollapsed}"));
