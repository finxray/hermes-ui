  ...
  try {
    ...
    if (!response?.ok()) {
      addResult(
        "route-load",
        "fail",
        `Long markdown fixture route returned HTTP ${response?.status() ?? "unknown"}. Restart the Web UI server if it predates this route.`
      );
      finalize();
      return;
    }
    ...
    if (ok) {
      if (candidate !== defaultBaseUrl) {
        addResult(
          ...
          "pass",
          `Using long markdown fixture route at ${candidate}; default ${defaultBaseUrl} did not serve it.`
        );
      }
      return candidate;
    }
...
async function hasFixtureRoute(candidate) {
  ...
  try {
    const response = await fetch(`${candidate}/design/markdown-long-fixture`, {
      cache: "no-store",
